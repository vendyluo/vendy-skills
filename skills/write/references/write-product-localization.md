...
- **Chinese**: Literal possessives such as "你的 Mac" or "你的裝置" when plain "Mac" or "本機" is enough; machine-output verbs such as "檢測到" when a result sentence would read better; mixed punctuation; English words with stable Chinese equivalents. Character-level half/full-width punctuation and CJK/Latin spacing are checked by `check_punctuation.py`; this list keeps the locale-voice judgment calls.
