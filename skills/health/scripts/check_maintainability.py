...
README_VERIFIER_RE = re.compile(
    r"^(?:python3?|bash|sh)\s+([^\s;&|]+)|^(\./[^\s;&|]+)"
)
VERIFIER_NAME_RE = re.compile(
    r"(test|check|lint|type|build|package|verify|validate|smoke)", re.IGNORECASE
)
...
# and run inside an arbitrary target project, so they import only stdlib.
# Do not hoist them into a shared scripts/ module: that would couple each
# standalone tool to this repository's install layout.
...
def parse_readme_commands(root: Path) -> list[str]:
    readme = next(
        (path for path in (root / "README.md", root / "README") if path.is_file()),
        None,
    )
    if readme is None:
        return []
    ...
    for raw in read_text(readme, 200_000).splitlines():
        stripped = raw.strip()
        if stripped.startswith("```"):
            if in_shell_fence:
                ...
                in_shell_fence = language in {"bash", "sh", "shell", "zsh", "console"}
            continue
        if not in_shell_fence or not stripped or stripped.startswith("#"):
            continue
        command = stripped.removeprefix("$ ").strip()
        match = README_VERIFIER_RE.match(command)
        if not match:
            continue
        target_text = match.group(1) or match.group(2)
        target = Path(target_text)
        if target.is_absolute() or ".." in target.parts or not VERIFIER_NAME_RE.search(target.name):
            continue
...
def verification_surface(
    ...
    commands = make_commands + package_commands + parse_ci_commands(root) + parse_readme_commands(root)
...
def hotspot_ownership_surface(
    ...
    findings: list[str] = []
    if missing:
        findings.append("large source files are candidates only; correlate with repeated failures, unclear boundaries, or verifier gaps before reporting")
    return ("INFO" if missing else "PASS"), documented, missing, findings
...
def main() -> int:
    ...
    if len(commands) >= 2 and not instruction_verification:
        wrapper_warnings.append(
            "multiple verification commands discovered but project instructions do not identify when to use them"
        )
    ...
    if not has_instruction_surface:
        context_warnings.append("no agent instruction surface (informational until current project evidence shows a need)")
    ...
    if not has_command_surface:
        verification_warnings.append("no executable verification command discovered; an honest manual runtime or artifact check may still exist")
    ...
    context_status = "INFO" if not has_instruction_surface else ("WARN" if context_warnings else "PASS")
    verification_status = "UNKNOWN" if not has_command_surface else ("WARN" if verification_warnings else "PASS")
