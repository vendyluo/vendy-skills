...
#   Optional memory path    -> built via sed on pwd; unusual chars produce wrong project key; verify manually if (none) seems wrong
...
#   Tier estimate           -> auto mode is provisional; confirm against collaborators, external effects, contracts, and rollback cost
set -euo pipefail

P=$(pwd)
SETTINGS="$P/.claude/settings.local.json"
TIER="${1:-auto}"
MODE="${2:-${HEALTH_MODE:-summary}}"
INCLUDE_MEMORY="${HEALTH_INCLUDE_MEMORY:-0}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [ "${HEALTH_DEEP:-0}" = "1" ]; then
  MODE="deep"
fi
case "$MODE" in
  summary|deep) ;;
...
# Produce only a provisional tier from mechanically visible signals.
# The agent confirms collaborators, production impact, contracts, and rollback cost.
if [ "$TIER" = "auto" ]; then
  if [ "${CONTRIBUTORS:-0}" -le 1 ] && [ "${CI_WORKFLOWS:-0}" -eq 0 ]; then
    TIER="simple"
  else
    TIER="standard"
  fi
fi
echo "provisional_tier: $TIER"
...
echo "[7/12] Optional continuity context..."
echo "=== OPTIONAL HANDOFF ==="
if [ "$MODE" = "deep" ]; then
  cat "$P/HANDOFF.md" 2>/dev/null || echo "(none)"
else
  print_file_summary "handoff_md" "$P/HANDOFF.md"
fi
echo "=== OPTIONAL MEMORY ==="
if [ -f "$HOME/.claude/projects/-${PROJECT_KEY}/memory/MEMORY.md" ]; then
  if [ "$MODE" = "deep" ] && [ "$INCLUDE_MEMORY" = "1" ]; then
    head -50 "$HOME/.claude/projects/-${PROJECT_KEY}/memory/MEMORY.md"
  else
    print_file_summary "memory_md" "$HOME/.claude/projects/-${PROJECT_KEY}/memory/MEMORY.md"
    [ "$MODE" = "deep" ] && echo "memory_content: not read (set HEALTH_INCLUDE_MEMORY=1 only with user approval)"
  fi
else
  echo "(none)"
fi
