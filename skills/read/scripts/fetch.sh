...
#   Default (no --use-proxy): local extractor only. The URL is requested from
#   its origin but is not sent to a third-party extraction service. Best quality
#   when readability-lxml + html2text are installed; degrades to a stdlib-only
#   stripper otherwise.
#
...
# Without --use-proxy, stop here rather than sending the URL to an extraction service.
