...
**Sample data:**
- Make sample or demo status unambiguous. Use structurally representative, non-identifying values; never make invented data look like current production truth.
...
Accessibility, interaction semantics, responsive integrity, and performance safety are correctness concerns. The exact visual recipes below are starting points and review heuristics, not universal laws.
...
- Prefer compositor-friendly `transform` and `opacity`. Animate other properties only when they express the transition better and the rendered result meets the actual performance budget
...
- Staggered enter: sequence semantic chunks only when the order helps comprehension. Tune delay to the content and keep the total sequence short; do not replay it on frequent refreshes or interactions
- Subtle exit: follow the destination and preserve continuity. A short displacement such as 12px over roughly 150ms can be an initial probe, not a fixed recipe; tune direction, distance, duration, and easing from the rendered surface. Exits are usually shorter and softer than entrances
- Contextual icon swaps: start with a short cross-fade when it preserves state clarity. Add scale, blur, rotation, or a spring only when it communicates the icon's semantic relationship and still reads clearly under interruption
- Press feedback: match the product's existing interaction language. Use scale only when it improves tactile feedback without destabilizing dense or frequent interactions; color, material, or opacity feedback may fit better
...
- Border radius coherence: reuse the project's current radii. For a self-contained new surface, choose a small coherent set locally. Introducing a named shared scale or redesigning existing radius behavior is a separate design-system decision.
...
Choose light or dark deliberately from the current product, audience, and context. Neither is universal; the table below is fallback inspiration when local evidence is absent, not a mandate.
...
If the answer is not obvious from the context, choose the simplest mode that fits the existing product and audience. Do not add a second theme or new token layer unless the user or current product contract requires it.

## Reflex Patterns to Challenge
...
Complements the temporal-design principles in the main SKILL.md. These are starting points for utility interfaces, not universal laws.

- Default to smooth ease-out in utility interfaces. Use bounce, elastic easing, or springs only when they fit the product's character or interaction physics; do not apply them as generic polish.
- Prefer `transform` and `opacity` for compositor-friendly motion. Other properties are valid when the visual relationship requires them and profiling shows the target surface remains responsive.
...
- Icon swaps: a short opacity cross-fade is a reliable starting point. Add scale or rotation only when it makes the semantic relationship clearer, then tune from the rendered surface rather than a fixed duration.
...
**Usage rule:** never auto-run the command. Mention it only when the user asks for a curated brand reference or current artifacts are insufficient. Run it with explicit approval, and treat the result as seed decomposition material, not a finished direction.
...
**Conflict resolution:** explicit user direction, the current product, accessibility, and real project constraints outrank the preset and this reference's aesthetic defaults. If the preset recommends a reflex font or generic pattern, keep it only when current evidence gives it a concrete role. State a meaningful override only when it affects review of the result.

Source: [github.com/VoltAgent/awesome-design-md](https://github.com/VoltAgent/awesome-design-md)

## Reference Material Priority

When source code and a screenshot are both available for a reference UI, inspect both. Source files provide exact tokens and component structure; screenshots provide rendered hierarchy, content fit, platform behavior, and the actual visual target. If they disagree, determine which artifact represents the intended current state rather than automatically privileging either one.
...
Use this scaffold only when the project already maintains DESIGN.md or the user requests a durable design artifact. Otherwise keep the necessary direction decisions in the current conversation. The nine available sections are:
...
Include only sections that make a real project decision; do not fill all nine for completeness. For a single component or quick prototype, skip this artifact.

## Pre-Handoff Checklist: Strategic Omissions

These are common product-surface gaps, not universal visual requirements. Check only the surfaces touched by the task and the jurisdictions or product contracts established by current project evidence. Report an out-of-scope gap instead of silently expanding the implementation.

- [ ] **404 path** when routing changed: preserve a clear route back into the product.
- [ ] **Back navigation** when the touched flow can create a dead end.
- [ ] **Form validation** when the task adds or changes inputs and their error states.
- [ ] **Skip-to-content** when the page has repeated navigation and no equivalent keyboard path.
- [ ] **Consent or legal surfaces** only when the project's actual jurisdiction, data behavior, or existing policy requires them.

When applicable, these are product or compliance requirements rather than visual polish. Keep them outside a visual-only implementation unless the current task authorizes that behavior.
...
When the user asks to pull a curated preset for a catalogued brand, offer `npx getdesign@latest add <brand>` and explain that it writes `DESIGN.md` into the project. Run it only with explicit approval, then decompose concrete properties from the result. A named brand alone does not require this dependency or artifact; screenshots, source tokens, and explicit direction are enough when already available.
...
- Match the project's existing press feedback; add `active:scale-95` only when scale feedback fits the interaction and motion constraints
- Keep button radius coherent within each existing component type
- Reuse the current radius scale. Introducing a new named scale or redesigning radius behavior is a separate design-system decision
...
- **Mix approaches when useful**: include a close-to-current option and a bolder departure when both are credible; do not invent an unexpected option merely to fill a slot
- **Progress from safe to bold** only when that range helps the user decide
- Accent-color swaps are not distinct options. Vary the layout, typeface, motion, or surface treatment when those are the real choices.

---

*Rules in Reflex Fonts, Font Selection, OKLCH, Theme Matrix, Reflex Patterns, Motion Specifics, and AI Slop Test adapted from [pbakaus/impeccable](https://github.com/pbakaus/impeccable) (Apache 2.0). DESIGN.md Scaffold adapted from [getdesign.md](https://getdesign.md) (MIT); concept credited to Google Stitch. Brand preset catalog from [VoltAgent/awesome-design-md](https://github.com/VoltAgent/awesome-design-md) (MIT). Content Authenticity, Multi-Card Alignment, and Strategic Omissions inspired by [Leonxlnx/taste-skill](https://github.com/Leonxlnx/taste-skill).*
